...
and LEDs on the hardware. So the Makefile also includes::
  ...
  USED_MODULES = .. module_xud module_usb_device module_usb_shared lib_mic_array_board_support ..
...
be included in your code to use the library. The low level USB device functionality is provided by ``xud.h``,
``usb_std_requests.h`` and ``usb_device.h``. USB descriptor set is defined in ``decriptors.h``. Access
to the Vocal Fusion board GPIO is provided by ``mic_array_board_support.h``.
