...
void endpoint0(chanend c_ep0_out, chanend c_ep0_in,
               client interface control i_control[1])
{
  ...
  while (1) {
    ...
    if (res == XUD_RES_OKAY) {
      ...
      debug_printf(
        "recipient %d type %d direction %d request %d value %d index %d length %d\n",
        sp.bmRequestType.Recipient, sp.bmRequestType.Type,
        sp.bmRequestType.Direction, sp.bRequest, sp.wValue,
        sp.wIndex, sp.wLength);

      switch ((sp.bmRequestType.Direction << 7) |
        ...
        case USB_BMREQ_H2D_VENDOR_DEV:
          ...
          if (res == XUD_RES_OKAY) {
            if (control_process_usb_set_request(sp.wIndex, sp.wValue,
                                                sp.wLength, request_data,
                                                i_control) == CONTROL_SUCCESS) {
        ...
        case USB_BMREQ_D2H_VENDOR_DEV:
           ...
           */
          if (control_process_usb_get_request(sp.wIndex, sp.wValue,
                                              sp.wLength, request_data,
                                              i_control) == CONTROL_SUCCESS) {
            len = sp.wLength;
            res = XUD_DoGetRequest(ep0_out, ep0_in, request_data, len, len);
            handled = 1;
            /* on control error, go to standard requests, which will
             * issue STALL
             */
          }
          break;
      ...
      if (!handled) {
        /* if we haven't handled the request about then do
         * standard enumeration requests
         */
...
int main(void)
{
  ...
  par {
    on USB_TILE: par {
      ...
      xud(c_ep_out, NUM_EP_OUT, c_ep_in, NUM_EP_IN,
