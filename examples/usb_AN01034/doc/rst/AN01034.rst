...
defined in ``mic_array_board_support.h`` which themselves are defined in the platform file ``MIC-ARRAY-1V0.xn``:
...
When the host requests a read from a controlled resource a setup packet to be sent ``xud()``. This is then forwarded to endpoint 0 where it is identified as a device-to-host vendor request 
...
When the host requests a write to a controlled resource a setup packet is received by ``xud()`` and then sent to endpoint 0 where it is identified as a host-to-device vendor request 
and is subsequently processed.
...
When running under a Windows platform, additionally a driver will be required to fully enumerate the USB device. See below for details.
